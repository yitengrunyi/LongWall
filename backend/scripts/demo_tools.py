...
在 backend 目录运行：
    ...
    .venv/bin/python scripts/demo_tools.py \
        --agent "读取 workspace 下的文件，然后用 web 搜索 LongWall"
...
async def _demo_direct() -> None:
    ...
    await executor.execute(
        ToolCall(
            id="q1",
            name="web_search",
            arguments={"query": "LongWall", "max_results": 3},
        )
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LongWall 工具层演示")
    parser.add_argument(
        "--direct",
        action="store_true",
        help="直接测试内置工具",
