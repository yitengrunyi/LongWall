...
async def _make_run_manager(tmp_path: Path, runtime: object) -> RunManager:
    ...
    run_store = SQLiteRunStore(tmp_path / "longwall.db")
    checkpoint_store = SQLiteCheckpointStore(tmp_path / "longwall.db")
...
async def test_send_message_persists_runtime_history_and_generates_title(
    ...
    store = SQLiteConversationStore(tmp_path / "longwall.db")
    await store.initialize()
    trace_store = SQLiteTraceStore(tmp_path / "longwall.db")
...
async def test_cli_lists_and_removes_conversation_permission_rules(
    ...
    store = SQLitePermissionRuleStore(tmp_path / "longwall.db")
    await store.initialize()
...
def test_cli_uses_provider_default_output_tokens_when_unspecified(
    ...
    monkeypatch.setattr(sys, "argv", ["longwall-chat"])
...
def test_cli_accepts_explicit_output_tokens(monkeypatch) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["longwall-chat", "--max-output-tokens", "8192"],
    )
...
def test_cli_prints_memory_list_and_details(capsys) -> None:
    ...
    memory = MemoryRecord(
        id="M001",
        title="LongWall 使用 SQLite 历史",
        summary="旧的 SQLite 记忆架构说明",
        content="LongWall 不再使用 SQLite + Embedding 作为长期记忆。",
        created_at=now,
        updated_at=now,
    ...
    assert "LongWall 使用 SQLite 历史" in output
...
def test_cli_parser_supports_short_startup_options(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["longwall", "-p", "deepseek", "-m", "deepseek-chat", "--new"],
    )
...
def test_cli_parser_supports_setup(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sys, "argv", ["longwall", "--setup"])
